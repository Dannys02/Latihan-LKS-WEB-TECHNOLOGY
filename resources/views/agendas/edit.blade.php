...
@section('content')
    <div class="max-w-2xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-md border border-gray-200">
        <h1 class="text-2xl font-bold tracking-tight text-gray-800 mb-6">Edit Data</h1>
        ...
        <form action="{{ route('agendas.update', $agenda->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="date" name="date" value="{{ old('date', $agenda->date) }}"
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        <option value="pending" {{ old('status', $agenda->status) == 'pending' ? 'selected' : '' }}>Pending
                        </option>
                        <option value="sedang berlangsung"
                            {{ old('status', $agenda->status) == 'sedang berlangsung' ? 'selected' : '' }}>Sedang
                            Berlangsung</option>
                        <option value="selesai" {{ old('status', $agenda->status) == 'selesai' ? 'selected' : '' }}>Selesai
                        </option>
                        <option value="dibatalkan" {{ old('status', $agenda->status) == 'dibatalkan' ? 'selected' : '' }}>
                            Dibatalkan</option>
                    </select>
                    @error('status')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" placeholder="Masukkan deskripsi"
                        class="mt-1 block w-full p-2 border rounded-md shadow-sm @error('description') border-red-500 @else border-gray-300 @enderror focus:ring-blue-500 focus:border-blue-500">{{ old('description', $agenda->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700">Gambar (Biarkan kosong jika tidak ingin
                        mengubah)</label>
